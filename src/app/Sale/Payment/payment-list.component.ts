...
export class PaymentListComponent implements OnInit {
    ...
    setupColumnDefs(): void {
        this.columnDefs = [
            { headerName: 'Customer', field: 'ledger', width: 100 },
            { headerName: 'Amount', field: 'amount', width: 80 },
            { headerName: 'Voucher No', field: 'voucherno', width: 80 },
            { headerName: 'Date', field: 'date', width: 80 },
            { headerName: 'Mode of Payment', field: 'modeofPayment', width: 80 },
            { headerName: 'Remark', field: 'remark', width: 80 },
          ...
          <div class="text-center">
            <button class="btn btn-warning me-1 btn-sm" 
              ...
              data-action="edit" data-id="${params.data.paymentId}" title="Edit">
              <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-danger btn-sm" 
              style="font-size:12px; padding:2px; width:18px; height:25px; margin-bottom:8px;" 
              data-action="delete" data-id="${params.data.paymentId}" title="Delete">
    ...
    onCellClicked(event: any): void {
        ...
        if (action === 'edit' && id) {
            this.router.navigate([`/Mainlayout/payment/edit/${id}`]); // This matches the new route
